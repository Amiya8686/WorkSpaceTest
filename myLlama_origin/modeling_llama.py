...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # 然后再转换 序列长度维度 和 头数目维度（方便后面进行因果注意力计算）
        # 得到的QKV的序列长度维度都是1
        ...
        # ## 改动: 存储ROPE前的KV
        # if(hasattr(past_key_values,"origin_kv")):
        #     origin_k, origin_v = past_key_values.origin_kv[self.layer_idx]["k"],past_key_values.origin_kv[self.layer_idx]["v"]
        #     if(origin_k is None):
        #         past_key_values.origin_kv[self.layer_idx]["k"] = key_states.clone()
        #     else:
        #         past_key_values.origin_kv[self.layer_idx]["k"] = torch.cat([origin_k,key_states],dim=2)
        #     if(origin_v is None):
        #         past_key_values.origin_kv[self.layer_idx]["v"] = value_states.clone()
        #     else:
        #         past_key_values.origin_kv[self.layer_idx]["v"]= torch.cat([origin_v,value_states],dim=2)
            
        


        # 旋转位置编码（这里的cos和sin是针对当前的Q和K的）
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if use_cache:
            ...
            # ## 存一下ROPE前的KV
            # if(not hasattr(past_key_values,"origin_kv")):
            #     past_key_values.origin_kv={}
            #     for i in range(self.config.num_hidden_layers):
            #         past_key_values.origin_kv[i]={"k":None,"v":None}
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        #验证使用的是本地的llama组件库
        print("🚀 [SUCCESS] 正在使用本地 myLlama 文件夹中的代码进行推理！")
